// [7/18/2012] Challenge #79 [easy] (Counting in steps)
// https://www.reddit.com/r/dailyprogrammer/comments/wvc21/7182012_challenge_79_easy_counting_in_steps/
// May 13, 2018

function step_count(start, end, steps){
  let arr = [];
  for (let i = 0; i < steps; i++){
    arr.push(start + (end - start) * i / (steps - 1));
  }
  return arr;
}


console.log(step_count(18.75, -22.00, 5));
//==> [18.75, 8.5625, -1.625, -11.8125, -22.0]

console.log(step_count(-5.75, 12.00, 5));
//==> [-5.75, -1.3125, 3.125, 7.5625, 12.0]

console.log(step_count(13.50, -20.75, 3));
//==> [13.5, -3.625, -20.75]

console.log(step_count(9.75, 3.00, 9));
//==> [9.75, 8.90625, 8.0625, 7.21875, 6.375, 5.53125, 4.6875, 3.84375, 3.0]
